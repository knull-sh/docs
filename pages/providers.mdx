...
| GCP VertexAI | `gcp_vertex` | Google Cloud VertexAI (Gemini) |
| GCP Anthropic | `gcp_anthropic` | Google Cloud VertexAI (Claude) |
...
| Field | Required | Description |
|-------|----------|-------------|
| `endpoint` | Yes | VertexAI endpoint |
| `gcpProject` | Yes | GCP project ID |
| `gcpLocation` | Yes | GCP region (e.g., `us-central1`) |
| `gcpAccessToken` | No | GCP access token |

### Authentication

Knull supports GCP authentication via:

1. **Workload Identity**: Use service account tokens
2. **Access Token**: Pass tokens directly
...
## GCP Anthropic (VertexAI)

### Claude Models on VertexAI

```yaml
models:
  - id: claude-3-5-sonnet@vertex_ai
